...
export default function TechStack({ techStackData }: TechStackProps) {
  ...
  const visibleTechs = showAll ? techStackData?.images : techStackData?.images?.slice(0, 4);
  return (
          ...
          {visibleTechs?.map((tech: any) => (
            <div key={tech.name} className="flex flex-col items-center">
              <div className="p-[2px] rounded-2xl bg-gradient-to-r from-[#ED0687] to-[#FE6C0F]">
                <div className="relative w-full max-w-[250px] aspect-[251/277] bg-white rounded-2xl shadow-md hover:shadow-xl transition-shadow flex ">
                  <Image
                    src={tech}
                    alt={tech}
                    width={120}
